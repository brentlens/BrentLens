...
export const StepSixExposure: React.FC<StepProps> = ({ onValidStateChange }) => {
  const { state, updateState }: any = useOnboarding();
  const entries = Object.entries(onboarding_mappings.fuel_exposure);

  useEffect(() => {
    onValidStateChange(!!state.fuelExposure);
  }, [state.fuelExposure, onValidStateChange]);
  ...
  return (
    <div className="step-view active animation-[fadeIn_0.25s_ease]">
      ...
      {/* Subdescription text content */}
      <p className="text-[14px] text-[var(--ink3)] leading-[1.7] mb-[24px]">
        How important is fuel to your operating costs? We use this to personalise cost impact forecasts.
      </p>

      {/* Grid List Container */}
      <div className="flex flex-col gap-2">
        {entries.map(([key, val]:any) => {
          const isSelected = state.fuelExposure === key;
          return (
            <div
              key={key}
              onClick={() => updateState({ fuelExposure: key, exposureLabel: val.label, exposureMultiplier: val.multiplier })}
