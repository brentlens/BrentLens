...
import { StepThreeCountry } from './StepThreeCountry';
import { StepFourScale } from './StepFourScale';
import { StepFiveSpend } from './StepFiveSpend';
import { StepSixExposure } from './StepSixGoal';
import { StepSevenHorizon } from './StepSevenHorizon';
import { StepEightStrategy } from './StepEightFeatures';
import { StepNineGoal } from './StepNineTeam';
import { Sun, Moon } from 'lucide-react';
import { saveUserOnboarding } from '@/services/user.service';
import { LoadingOverlay } from './LoadingOverlay';
import { SuccessOverlay } from './SuccessOverlay';
import { StepTenSubscription } from './StepTenSubscription';
...
export const OnboardingHub: React.FC = () => {
  ...
  const handleNext = async () => {
    ...
    if (state.step < totalSteps) {
      ...
      const res = await saveUserOnboarding(state);
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-full">
        ...
        <main className="flex-1 overflow-y-auto p-8 pb-32 scroll-clean bg-[var(--bg)]">
			<div className="max-w-3xl mx-auto  p-8  relative">
				...
				{state.step === 3 && <StepThreeCountry onValidStateChange={setIsValid} />}
				{state.step === 4 && <StepFourScale onValidStateChange={setIsValid} />}
				{state.step === 5 && <StepFiveSpend onValidStateChange={setIsValid} />}
				{state.step === 6 && <StepSixExposure onValidStateChange={setIsValid} />}
				{state.step === 7 && <StepSevenHorizon onValidStateChange={setIsValid} />}
				{state.step === 8 && <StepEightStrategy onValidStateChange={setIsValid} />}
				{state.step === 9 && <StepNineGoal onValidStateChange={setIsValid} />}
				{state.step === 10 && <StepTenSubscription onValidStateChange={setIsValid} />}
